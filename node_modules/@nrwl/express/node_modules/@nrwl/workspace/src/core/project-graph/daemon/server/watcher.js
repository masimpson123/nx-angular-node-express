...
const shutdown_utils_1 = require("@nrwl/workspace/src/core/project-graph/daemon/server/shutdown-utils");
...
const ALWAYS_IGNORE = [
    ...
    (0, path_1.join)(app_root_1.appRootPath, '.git'),
    socket_utils_1.FULL_OS_SOCKET_PATH,
];
function getIgnoredGlobs() {
    return [
        ...ALWAYS_IGNORE,
        ...getIgnoredGlobsFromFile((0, path_1.join)(app_root_1.appRootPath, '.nxignore')),
        ...getIgnoredGlobsFromFile((0, path_1.join)(app_root_1.appRootPath, '.gitignore')),
    ];
}
function getIgnoredGlobsFromFile(file) {
    try {
        return (0, fs_1.readFileSync)(file, 'utf-8')
            ...
            .map((i) => (i.startsWith('/') ? (0, path_1.join)(app_root_1.appRootPath, i) : i));
    }
    catch (e) {
        return [];
    }
...
function configureIgnoreObject() {
    ...
    try {
        ig.add((0, fs_1.readFileSync)(`${app_root_1.appRootPath}/.gitignore`, 'utf-8'));
    }
    catch (_a) { }
    try {
        ...
        const subscription = yield watcher.subscribe(app_root_1.appRootPath, (err, events) => {
            ...
            for (const event of events) {
                const workspaceRelativeEvent = {
                    type: event.type,
                    path: (0, devkit_1.normalizePath)((0, path_1.relative)(app_root_1.appRootPath, event.path)),
                };
