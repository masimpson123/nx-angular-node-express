...
function modifyJestConfig(host, context, project, setupFile, jestConfig, tsConfig, isAngular) {
    ...
    try {
        const jestObject = (0, require_jest_config_1.getJestObject)(`${app_root_1.appRootPath}/${jestConfig}`);
        if (setupFile !== '') {
            // add set up env file
            // setupFilesAfterEnv
            ...
            (0, update_config_1.addPropertyToJestConfig)(host, jestConfig, 'setupFilesAfterEnv', setupFilesAfterEnv);
        }
        ...
        if (!existingGlobals) {
            (0, update_config_1.addPropertyToJestConfig)(host, jestConfig, 'globals', {
            ...
            if (!checkJestPropertyObject(existingGlobalTsJest)) {
                (0, update_config_1.addPropertyToJestConfig)(host, jestConfig, 'globals.ts-jest', globalTsJest);
            }
        }
    }
    catch (_a) {
    ...
    * globals.ts-jest with:
    "${(0, devkit_1.serializeJson)(globalTsJest)}"
  `);
    }
}
...
function update() {
    return (0, schematics_1.chain)([updateJestConfigForProjects(), (0, workspace_1.formatFiles)()]);
}
