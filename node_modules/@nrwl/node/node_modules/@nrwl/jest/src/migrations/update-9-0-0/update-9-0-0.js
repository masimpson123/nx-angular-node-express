...
function update() {
    return (0, schematics_1.chain)([
        displayInformation,
        (0, workspace_1.updatePackagesInPackageJson)(path.join(__dirname, '../../../', 'migrations.json'), '9.0.0'),
        updateJestConfigs,
...
function displayInformation(host, context) {
    const config = (0, ast_utils_1.readJsonInTree)(host, 'package.json');
...
function updateJestConfigs(host) {
    const config = (0, ast_utils_1.readJsonInTree)(host, 'package.json');
    if (config.devDependencies && config.devDependencies['jest-preset-angular']) {
        const workspaceConfig = (0, workspace_1.readWorkspace)(host);
        ...
        jestConfigsToUpdate.forEach((configPath) => {
            if (host.exists(configPath)) {
                ...
                const changes = [];
                (0, ast_utils_1.getSourceNodes)(sourceFile).forEach((node) => {
                ...
                (0, workspace_1.insert)(host, configPath, changes.sort((a, b) => (a.order > b.order ? -1 : 1)));
            }
        });
    }
}
