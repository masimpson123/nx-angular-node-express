...
class CLIOutput {
    ...
    applyNxPrefix(color = 'cyan', text) {
        let nxPrefix = '';
        if (chalk[color]) {
            nxPrefix = `${chalk[color]('>')} ${chalk.reset.inverse.bold[color](' NX ')}`;
        }
        else {
            nxPrefix = `${chalk.keyword(color)('>')} ${chalk.reset.inverse.bold.keyword(color)(' NX ')}`;
        }
    ...
    logCommand(message, taskStatus) {
        ...
        if (taskStatus === 'local-cache') {
            ...
            commandOutput += `  ${chalk.grey('[existing outputs match the cache, left as is]')}`;
        }
