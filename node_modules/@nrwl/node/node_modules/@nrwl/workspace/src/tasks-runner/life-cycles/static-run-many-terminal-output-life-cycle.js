    ...
    endCommand() {
        ...
        if (this.failedTasks.length === 0) {
            ...
            if (skippedTasks.length > 0) {
                bodyLines.push(output_1.output.colors.gray('Tasks not run because their dependencies failed:'), '', ...skippedTasks.map((task) => `${output_1.output.colors.gray('-')} ${task.id}`), '');
            }
            bodyLines.push(output_1.output.colors.gray('Failed tasks:'), '', ...[...this.failedTasks.values()].map((task) => `${output_1.output.colors.gray('-')} ${task.id}`));
