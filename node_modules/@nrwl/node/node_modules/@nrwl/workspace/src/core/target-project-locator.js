...
class TargetProjectLocator {
    ...
    getAbsolutePath(path) {
        return (0, path_1.join)(app_root_1.appRootPath, path);
    }
    getRootTsConfig() {
        let path = 'tsconfig.base.json';
        let absolutePath = this.getAbsolutePath(path);
        let content = (0, file_utils_1.readFileIfExisting)(absolutePath);
        if (!content) {
            path = 'tsconfig.json';
            absolutePath = this.getAbsolutePath(path);
            content = (0, file_utils_1.readFileIfExisting)(absolutePath);
        }
        if (!content) {
            return {
                path: null,
        ...
        return { path, absolutePath, config: (0, devkit_1.parseJson)(content) };
    }
    findMatchingProjectFiles(file) {
        for (let currentPath = file; currentPath != (0, path_1.dirname)(currentPath); currentPath = (0, path_1.dirname)(currentPath)) {
