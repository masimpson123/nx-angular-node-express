    ...
    const dependencies = recursivelyCollectDependencies(projectName, projGraph, [])
        ...
        if (depNode.type === project_graph_1.ProjectType.lib) {
            if (isBuildable(targetName, depNode)) {
                const libPackageJson = (0, devkit_1.readJsonFile)((0, path_1.join)(root, depNode.data.root, 'package.json'));
                return {
                    name: libPackageJson.name,
                    outputs: (0, utils_1.getOutputsForTargetAndConfiguration)({
                        overrides: {},
                        target: {
                            project: projectName,
                            target: targetName,
...
function updateBuildableProjectPackageJsonDependencies(root, projectName, targetName, configurationName, node, dependencies, typeOfDependency = 'dependencies') {
    const outputs = (0, utils_1.getOutputsForTargetAndConfiguration)({
        overrides: {},
        target: {
            project: projectName,
            target: targetName,
    ...
    dependencies.forEach((entry) => {
        var _a;
        const packageName = (0, project_graph_1.isNpmProject)(entry.node)
        ...
        if (!hasDependency(packageJson, 'dependencies', packageName) &&
            ...
            try {
                ...
                else if ((0, project_graph_1.isNpmProject)(entry.node)) {
