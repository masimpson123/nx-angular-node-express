...
function workspaceGenerators(args) {
    return (0, tslib_1.__awaiter)(this, void 0, void 0, function* () {
        ...
        process.exitCode = yield (0, generate_1.generate)(process.cwd(), rootDirectory, args, parsedArgs.verbose);
    });
}
...
function compileTools() {
    ...
    (0, devkit_1.writeJsonFile)(path.join(generatorsOutDir, 'workspace-generators.json'), collectionData);
    return generatorsOutDir;
}
function getToolsOutDir() {
    const outDir = toolsTsConfig().compilerOptions.outDir;
    if (!outDir) {
        devkit_1.logger.error(`${toolsTsConfigPath} must specify an outDir`);
        process.exit(1);
    }
...
function constructCollection() {
    ...
    (0, fs_1.readdirSync)(generatorsDir).forEach((c) => {
        ...
        if ((0, fs_1.existsSync)(path.join(childDir, 'schema.json'))) {
            generators[c] = {
                factory: `./${c}`,
                schema: `./${(0, devkit_1.normalizePath)(path.join(c, 'schema.json'))}`,
...
function toolsTsConfig() {
    return (0, devkit_1.readJsonFile)(toolsTsConfigPath);
}
function listGenerators(collectionFile) {
    try {
        const bodyLines = [];
        const collection = (0, devkit_1.readJsonFile)(collectionFile);
...
function parseOptions(args, outDir) {
    ...
    if ((0, fileutils_1.fileExists)(schemaPath)) {
        const { properties } = (0, devkit_1.readJsonFile)(path.join(outDir, args[0], 'schema.json'));
        if (properties) {
...
function createTmpTsConfig(tsconfigPath, updateConfig) {
    ...
    const originalTSConfig = (0, devkit_1.readJsonFile)(tsconfigPath);
