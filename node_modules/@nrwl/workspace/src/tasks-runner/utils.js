...
function getCommandAsString(task) {
    const execCommand = (0, devkit_1.getPackageManagerCommand)().exec;
...
function getExecutorNameForTask(task, workspace) {
    ...
    if ((0, fs_1.existsSync)((0, path_1.join)(project.root, 'package.json'))) {
        project.targets = (0, project_graph_utils_1.mergeNpmScriptsWithTargets)(project.root, project.targets);
    }
...
function shouldForwardOutput(task, initiatingProject, options) {
    if (process.env.NX_FORWARD_OUTPUT === 'true')
        return true;
    if (!isCacheableTask(task, options))
        return true;
    if (task.target.project === initiatingProject)
        return true;
    return false;
}
exports.shouldForwardOutput = shouldForwardOutput;
