...
function createRunOneDynamicOutputRenderer({ initiatingProject, tasks, args, overrides, }) {
    return (0, tslib_1.__awaiter)(this, void 0, void 0, function* () {
        ...
        const renderDependentTargets = (renderDivider = true) => {
            ...
            switch (state) {
                case 'EXECUTING_DEPENDENT_TARGETS':
                    if (totalFailedTasks === 0) {
                        linesToRender.push(`   ${output_1.output.colors.cyan(cli_spinners_1.dots.frames[dependentTargetsCurrentFrame])}    ${output_1.output.colors.white.dim(`Nx is waiting on ${remainingDependentTasksNotFromInitiatingProject} dependent project tasks before running tasks from`)} ${output_1.output.colors.white(`${initiatingProject}`)}...`);
            ...
            if (totalSuccessfulTasks > 0) {
                linesToRender.push(output_1.output.colors.gray(`   ${output_1.output.colors.gray(figures.tick)}    ${totalSuccessfulTasks}${`/${totalCompletedTasks}`} dependent project tasks succeeded ${output_1.output.colors.gray.dim(`[${totalCachedTasks} read from cache]`)}`));
            }
            ...
            for (const task of tasks) {
                ...
                if (task.target.project === initiatingProject &&
                    ...
                    if (totalDependentTasksNotFromInitiatingProject > 0) {
                        ...
                        process.stdout.write(`   ${output_1.output.colors.gray('Hint: you can run the command with')} --verbose ${output_1.output.colors.gray('to see the full dependent project outputs')}` + os_1.EOL);
