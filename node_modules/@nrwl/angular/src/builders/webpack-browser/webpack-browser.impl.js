...
function buildApp(options, context) {
    const { customWebpackConfig } = options, delegateOptions = (0, tslib_1.__rest)(options, ["customWebpackConfig"]);
...
function run(options, context) {
    const { target, dependencies } = (0, buildable_libs_utils_1.calculateProjectDependencies)((0, project_graph_1.readCachedProjectGraph)(), context.workspaceRoot, context.target.project, context.target.target, context.target.configuration);
    options.tsConfig = (0, buildable_libs_utils_1.createTmpTsConfig)((0, path_1.join)(context.workspaceRoot, options.tsConfig), context.workspaceRoot, target.data.root, dependencies);
    process.env.NX_TSCONFIG_PATH = options.tsConfig;
    return (0, rxjs_1.of)((0, buildable_libs_utils_1.checkDependentProjectsHaveBeenBuilt)(context.workspaceRoot, context.target.project, context.target.target, dependencies)).pipe((0, operators_1.switchMap)((result) => {
        if (result) {
